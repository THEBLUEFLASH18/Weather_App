...
let splineUrl = 'https://prod.spline.design/HpeS6JEAKu9X-nUy/scene.splinecode'; // Default URL
app.load(splineUrl); // Load the default scene
...
submitButton.addEventListener('click', async function weatherInfo() {
    ...
    if (weatherData.ok) {
        ...
        // Update splineUrl based on weather description
        if (newData.weather[0].description === 'clear sky') {
            splineUrl = 'https://prod.spline.design/p9FyhN5Pd0xgAFwl/scene.splinecode';
        } else if (
            newData.weather[0].description === 'few clouds' ||
            newData.weather[0].description === 'scattered clouds' ||
            newData.weather[0].description === 'broken clouds' ||
            newData.weather[0].description === 'overcast clouds'
        ) {
            ...
            splineUrl = 'https://prod.spline.design/HpeS6JEAKu9X-nUy/scene.splinecode';
        } else if (
        ...
        try {
            ...
            console.error('Error loading Spline scene:', loadError);
        }
    } else {
